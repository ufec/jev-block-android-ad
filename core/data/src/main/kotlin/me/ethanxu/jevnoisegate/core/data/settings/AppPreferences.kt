...
data class AppPreferences(
    ...
    // --- 判定门槛与动作 ---
    /**
     * 各类别的判定门槛覆盖值，键是类别名（如"广告"）。
     *
     * 门槛的含义：模型给出类别与置信度后，**置信度低于此值就不执行动作**，退回放行。
     * 它是防止"模型随口一说就把用户的消息吃掉"的那道闸。
     *
     * 存的是**覆盖值**而不是完整配置：答案空间（有哪些类别、各自描述什么）
     * 仍由 `DefaultCategories` 唯一定义，用户只调这两个旋钮。
     * 空 map 表示全部沿用出厂默认 —— 于是以后调整出厂值，没动过的用户会跟着变，
     * 而不是被一份陈旧的快照钉死在旧行为上。
     *
     * 注意放行类（动作是 ALLOW 的类别）的门槛**没有作用**：置信度低于门槛走降级放行，
     * 高于门槛执行该类别动作 —— 而它的动作本来就是放行，两条路结果一致。
     */
    val categoryThresholds: Map<String, Float> = emptyMap(),

    /**
     * 各类别的动作覆盖值，键是类别名，值是 `Action` 的枚举名（如 `SILENT_SUPPRESS`）。
     *
     * 存枚举名而非枚举本身，理由同本文件顶部：`:core:data` 不反向依赖上层，
     * 解析留给读取侧（见 `DispatchModule`），非法名字在那里降级为出厂动作。
     *
     * 空 map 表示全部沿用出厂默认。
     */
